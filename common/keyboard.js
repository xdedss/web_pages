...
(function($, kb){
    ...
    kb.onKeyDown = function(keycode, callback){
        if (keycode instanceof Array){
            keycode.forEach(kc=>kb.onKeyDown(kc, callback));
        }
        else{
            if (typeof(keycode) == "string" && namemap[keycode] != null) keycode = namemap[keycode];
            ...
            listeners[keycode].down.push(listener);
            return listener;
        }
    };
    ...
    kb.onKeyUp = function(keycode, callback){
        if (keycode instanceof Array){
            keycode.forEach(kc=>kb.onKeyUp(kc, callback));
        }
        else{
            if (typeof(keycode) == "string" && namemap[keycode] != null) keycode = namemap[keycode];
            if (listeners[keycode] == null){
            ...
            listeners[keycode].up.push(listener);
            return listener;
        }
    };
    
    kb.getKey = function(keycode){
        if (typeof(keycode) == "string" && namemap[keycode] != null) return keymap.get(namemap[keycode]);
